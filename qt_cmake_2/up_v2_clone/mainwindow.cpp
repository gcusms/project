...
//        }
        if(!useLocal_t)
        {
            ...
            x.push_back(sum_t);
//            customPlot->xAxis->setRange(sum_t - sum_t%1000,sum_t);
//            qDebug()<<"x  = "<<x.at(x.size() -1);
        }else
        {
        ...
        customPlot->yAxis->setRange(MIN_y,MAX_y);//重新设置y的范围
        customPlot->xAxis->setRange(customPlot->xAxis->range().upper, 5, Qt::AlignRight);  // 移动
         customPlot->yAxis->setRange(customPlot->yAxis->range().upper, MAX_y, Qt::AlignRight);  // 移动
//        customPlot->graph(0)->rescaleAxes(true);  //回到最佳的位置
        customPlot->graph(0)->setData(x,y);  //画线函数
        customPlot->replot();
        //显示数据
...
void MainWindow::UpdateDate( int y_input, int grapNumber,bool flag)
{
    ...
    if(flag)
    {
          RM_AddPoints(y_input,grapNumber,false);
    }else
    {
            y_input = 0;
    }
    ...
    customPlot->xAxis->setRange(MIN_x,MAX_x);
    customPlot->yAxis->setRange(MIN_y,MAX_y);
    customPlot->xAxis->setRange(customPlot->xAxis->range().upper, 5, Qt::AlignRight);  // move
    customPlot->graph(0)->setData(x,y);  //画线函数
    customPlot->replot();
 ...
 * 输入的点
*/
void MainWindow::RM_AddPoints(double y_input, int grapNumber,bool input_flag)
{
    if(input_flag)
    {
        if( y_input > MAX_y)
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Escape:
        ...
        break;
    case Qt::Key_A:
        ui->pushButton_tstart->click();//开始按键点击
        break;
    case Qt::Key_B:
        ui->pushButton_tend->click();//停止按键点击
        break;
    case Qt::Key_R:
        RM_BEGIN = !RM_BEGIN;
        break;
    case Qt::Key_T:
        test_flag = !test_flag;
        qDebug()<<"T clicked"<<"test_falg = "<<test_flag;
        break;
    case Qt::Key_F1:
        qDebug()<<" A : 开始";
        qDebug()<<" B: 暂停";
